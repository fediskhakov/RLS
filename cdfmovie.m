...
function res = cdfmovie (par,mp,sw,parameter,interval,filename,xlow, dummystr, lbl, graphstring)
...
nsteps=1000; %number of steps/frames in the movie
nsteps_nodisk=120;
if isempty(filename)
    SAVETODISK=0;
else
...
if true
    fprintf('Compiling... '); 
    tic
    mex -largeArrayDims leapfrog.c -DMAXEQB=5 -DPRINTeqbloop=0 -DPRINTeqbstr=0
...
% mfig= figure('Color',[1 1 1],'NextPlot','new');
if SAVETODISK
    %start movie
...
for parametervalue=interval(1):(interval(2)-interval(1))/(nsteps-1):interval(2)
    ...
    eqbstr=eqbstr';
    v10_mon=gm(par.nC).solution(1,7);
    eqbstr(:,8)=eqbstr(:,8)/v10_mon;

    delete(allchild(mfig));%clean the figure
    % plot
    if isempty(graphstring)
        graph.EqbstrCDFPlot(eqbstr,[0 -5 6 7],dummystr, lbl, mp,sw,'Distribution of found equilibria', xlow, mfig);
    else
        eval(graphstring)    
    end
