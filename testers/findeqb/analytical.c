...
void analytical_solutions (int ic1,int ic2,int iC,MPstruct mp,MVstruct mv,Bnestruct bne,Gamestruct* g,Brstruct* br) {
    /*--------------------------------------------------------------------------------------
    ...
    int ieqb, neqb;
    int save;
    double a0[2], a1[2], a2[2], pstar[4][2],v1p[2], A[2], B[2], v0_0[2], v0_1[2], pbr[2], v0p[2]; 
    int ir;
    ...
    //Coefficients to A    
    br[iC].A0[0][brOUT]=bne.pf1[IdBNE]+mp.df*mv.pti*mv.h1[ih(nC, ic1, ic2, iC)];
    br[iC].A0[1][brOUT]=bne.pf2[IdBNE]+mp.df*mv.pti*mv.h2[ih(nC, ic1, ic2, iC)];

    br[iC].A1[0][brOUT]=mp.df*(
            mv.pti*mv.h1[ih(nC, ic1, iC, iC)]  + (1-mv.pti)*f_lnsum(nF, mp.eta, g[iC].v10[i1], g[iC].v11[i1])
            -mv.pti*mv.h1[ih(nC, ic1, ic2, iC)]
            );
    br[iC].A1[1][brOUT]=mp.df*(
            mv.pti*mv.h2[ih(nC, iC, ic2, iC)]  + (1-mv.pti)*f_lnsum(nF, mp.eta, g[iC].v20[i2], g[iC].v21[i2])
            -mv.pti*mv.h2[ih(nC, ic1, ic2, iC)]
            );

    //Coefficients to B
    br[iC].B1[brOUT]=mp.df*(1-mv.pti);

    //Coefficients to value of investing
    br[iC].C0[0][brOUT]=bne.pf1[IdBNE]-mv.kv
            + mp.df*( mv.pti*mv.h1[ih(nC, iC, ic2, iC)] + (1-mv.pti)*f_lnsum(nF, mp.eta, g[iC].v10[i2],g[iC].v11[i2])) ;
    br[iC].C0[1][brOUT]=bne.pf2[IdBNE]-mv.kv
            + mp.df*( mv.pti*mv.h2[ih(nC, ic1, iC, iC)] + (1-mv.pti)*f_lnsum(nF, mp.eta, g[iC].v20[i1],g[iC].v21[i1])) ;
    br[iC].C1[0][brOUT]=mp.df*(
             ( mv.pti*mv.h1[ih(nC, iC, iC, iC)]  + (1-mv.pti)*f_lnsum(nF, mp.eta, g[iC].v10[id(nC, iC, iC, iC, 0)],g[iC].v11[id(nC, iC, iC, iC, 0)])  ) -
             ( mv.pti*mv.h1[ih(nC, iC, ic2, iC)] + (1-mv.pti)*f_lnsum(nF, mp.eta, g[iC].v10[i2],g[iC].v11[i2])  )
            );
    br[iC].C1[1][brOUT]=mp.df*(
             ( mv.pti*mv.h2[ih(nC, iC, iC, iC)]  + (1-mv.pti)*f_lnsum(nF, mp.eta, g[iC].v20[id(nC, iC, iC, iC, 0)],g[iC].v21[id(nC, iC, iC, iC, 0)])  ) -
             ( mv.pti*mv.h2[ih(nC, ic1, iC, iC)] + (1-mv.pti)*f_lnsum(nF, mp.eta, g[iC].v20[i1],g[iC].v21[i1])  )
             );
...
//printf("iF=%d a0=%f a1=%f a2=%f roots= %f %f \n",iF,a0[iF],a1[iF],a2[iF],pstar[1][iF],pstar[2][iF]);
        ...
        for (j=0;j<4;j++) {
            for (i=0;i<4;i++) {
                ...
                if (((i==0) || (i==3)) && ((j==0)  || (j==3))) { // corner
                    //best responce for firm 1: 
                    //opponent invests with prob 0 or 1 = pstar[i][0] => if polynomial>0 invest with prob 0, otherwise with prob 1              
                    ...
                    if ((pbr[0]==pstar[j][1]) && (pbr[1]==pstar[i][0])){
                        ieqb=ieqb+1;
                        save=1;
                    }
                }
                if (((i==1) || (i==2)) && ((j==1) || (j==2))) { // mixed strategy
//printf("ic=%d ic1=%d ic2=%d ieqb=%d i=%d j=%d pstar[1][0]=%f pstar[1][1]=%f pstar[2][0]=%f pstar[2][1]=%f\n",iC,ic1,ic2,ieqb,i,j,pstar[1][0],pstar[1][1],pstar[2][0],pstar[2][1]);
                    //check if pstar lies in unit interval
...
//printf("condition2\n");
                ...
                if (ieqb>=MAXEQB) mexErrMsgTxt("Error in analytical.c, eta==0: number of equilibria exceeded MAXEQB");
        ...
        for (ieqb=0;ieqb<neqb;ieqb++) {
            g[iC].seleqb[id(nC,ic1,ic2,iC,ieqb)]=(ieqb==f_ers(iC,ic1,ic2,neqb)?1.0:0.0);
        }
    }
    else
    {   //eta>0
...
//        printf("search in interval [l,r]=[%f, %f] starting from m=%f \n\n", l, r, m);
        ...
        if (retcode == 0) {
            ...
            if (neqb > MAXEQBINFO) mexErrMsgTxt("Error in findeqb.c: located equilibria exceed MAXEQBINFO");
...
*/
    ...
    if (D>=0) {
        ...
        if ((p_j)>0 && (p_j)<1) {
            signinvbr2=brc[0].r1; //if no roots, p_i is too low for positive root and too high for negative root
            ...
            D=a1*a1-4*a0*a2;
            if (D>=0) {
                signinvbr2=-brc[0].r1; //if outside of unit interval p_i is too low for negative root and too high otherwise
